...
class LobbyHandler {
  ...
  // sina egna matcher och matcher man deltar i
  ...
  async joinGame(user, gameId) {
    return new Promise((resolve, reject) => {
      db.getGameById(gameId).then((gamemodel) => {
        if (user.id === gamemodel.user1) {
          socketEventHandler.joinedGame(gameId);
          resolve();
        } else if (gamemodel.user2 === null) {
          ...
          console.log("could not join game because it is already full");
          resolve();
        }
      }).catch((err) => {
        console.error(`Could not fetch game by its game ID : ${gameId}, error: ${err.message}`)
        reject(err);
      });
    })
  }
